...
> **상태**: **운영 중** — 09-07 첫 자동 체인에서 상류(2~3단계) 실패 → G1 보유 유지 스냅샷 정상 기록 (실패 설계 첫 실전 검증, primary +1.83% / SPY +0.41%). 정상 경로 전 구간 첫 완전 성공은 **09-14 실행에서 확인**
>
> **v0.3 변경 (구현 완료·배포)**:
...
- [x] **4단계 부록 A 상호 확정** (ff83d59, 2026-09-03): "target 부재 = 보유 유지" §5 G1 확정,
  04 부록 A 확정 표기. 09-07 실운영에서 첫 발동 (상류 실패 → 보유 유지 스냅샷) — 04 v0.4 G5 와 짝
