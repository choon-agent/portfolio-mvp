...
- 2026-08-10 (정기 · **v0.17 bear cap primary 첫 정기 실행**): 20/20 ok, 재시도 0, $0.37. turnover 3/20 (빠짐 WDC/CFG/NEM, 들어옴 NEE/MGM/ADM). **위반 1/20** (직전 3주 7/7/6 → 1 — v0.17 목표 실운영 달성). 음수 skew **8/20(40%)** — regime 이래 최저, 분포 건전화. bear_uncapped counterfactual 정상 작동: 구 단골 종목의 왜곡분 가시화 (EIX primary +4.3% vs uncapped +58.0% / UAL +1.5% vs +31.4% / DAL +0.8% vs +19.7%). **신규 관찰 — DD 극소 양수 TTM EPS 유형**: 08-04 발표 분기 대규모 일회성 손실로 TTM EPS 0.45 (current 142.47, P/E ~317) → `ttm_eps>0` 가드 통과하나 peer 목표가 10~12 로 붕괴 → bull=11.89 < bear=29.25(hist), ER **-88.5%**. cap 무관 유형 (bear ≤ current 라 미발동). 방향은 보수적(과대 매도 신호)이고 flag 로 calibration 자동 제외되나 분포 오염 — §12.3 잔여 관찰 (d) 로 추가 (가드 후보: current/ttm_eps 극단 시 peer 갈래를 결측 취급). trigger batch 재실행: 신규 발표 0, 기존 163 스킵 (멱등성 확인).
...
- 2026-09-12 (**trigger batch 주간 실행 — 08-17 이후 첫 실행, 3주 밀림**): 대상 260쌍 중 already 168 / **신규 16** (TGT·ROST 8월 말 발표) / not_filed_yet 76 (08-17~08-31 시나리오는 10~11월 발표 대기). 채점 48건 — met 14, 평가불가 5 (earnings_surprise 4·fcf_yoy 1). **fcf_yoy 남발 재확인** (TGT fcf +141% vs thr 20 — 4개 주차 동일 발동). calibration 16건: realized bull 11 / base 5, Brier 0.697 (표본 소, 유효 regime 07-14 이후분만 §1.4.2 산입). 월간 비용 리포트 08 확정본 재생성 ($5.59 LLM / AWS $7.99, 09 진행분 LLM $0 — 09-07 전건 즉시 실패). **교훈**: 배치·리포트가 로컬 수동이라 밀림 — §0.8 C 의 Lambda 자동화 결정 근거 추가.
- 2026-09-14 (정기 · **1~5단계 첫 완전 성공** — SDK 복구 후): 스크리닝 480→20, Bull/Bear 40/40 ok 재시도 0 ($0.71) + 시나리오 20/20 ($0.37) = **$1.08/주**. 위반 2/20 — DD 6주 연속 (d) + **GM 신규 (a) base<bear 유형** (peer P/E 중앙값 27×EPS 1.98 = base 53.64 < 현재가 85.62, historical bear 55.35 > base — TKO/ADM 계열, cap 무관). 음수 skew 6/20(30%, regime 최저). **optimizer 후보 12 → 첫 완전 투자 (현금 0%)**, 9종목 (ALL/NEE/NEM/WTW 15% cap — 포지션 수 9 < 하한 10, CHARTER v0.2 허용). primary ER 3.49% vs baseline 4.24% (4주 연속 baseline 우위). **rebalancer 첫 정상 경로 체결**: 매매 13건 (exit 4 CF/DVA/EIX/HST + 신규 5 EXPE/NEE/NEM/SPG/TGT), **one-way 턴오버 ≈68%** (2주치 드리프트 + 유니버스 교체 — 09-07 결번 효과 포함이나 종목 교체 churn 이 몸통. 05 §8 "신규 편입 지연 규칙" 데이터 게이트 1건 추가). 주간: primary +0.12% / option_b -0.16% / SPY -0.77%. **누적 08-17~09-14 (4주)**: primary -2.71% / option_b -2.71% / SPY -1.09%. 컨테이너 Lambda 2개 로그 첫 정상 기록 (정책 v3 실전 검증). #3 실현 표본 4주.
- 2026-09-14/15 (**모델 이행: Sonnet 4.6 → Sonnet 5**, b5892ab — 2·3단계 LLM. **regime 마커**: 09-21 실행부터 Bull/Bear·시나리오 산출이 Sonnet 5): 이행 검증 — `temperature` 미전송(Sonnet 5 는 sampling 파라미터 400)·`thinking: disabled` 명시(생략 시 adaptive 과금)·단가 $2/$10. Bull/Bear 골든 8/8 + 시나리오 골든 4/4 실호출 성공(narrative 500 제한 내), golden 56건 통과, CI 배포 09-14 15:49Z 확인. 토크나이저 +21~27% 토큰이나 단가 인하로 호출당 -18% → **주간 ≈ $0.87** (4.6 시절 $1.08). **DeepEval 회귀: 16/24 (baseline 32/32)** — `signals_not_primary_evidence` 4/8 (z-score 를 1차 근거로 인용), 02 §11.5 기록·프롬프트 보강 안건. 12주 판정 표본에 모델 경계 존재 (07-14 epsDiluted 경계와 동급으로 표기). **09-21 관전**: attempts>1 비율(스키마 실패), stop_reason=max_tokens, flags, 확률 분포(상수성 해소 여부), 비용.
...
## 0.8 M3 말 재검토 안건 (~10월 초 — 유효 12주 판정과 동시 실행)

> 시점: 유효 regime(07-14~) 12주 도달 ≈ **2026-10-06 주**. CHARTER §7 "M3 말: Charter
> 재검토, 실전 전환 판단, 블로그 1편 초고" + 흩어진 이월 항목을 여기로 일원화.
> (2026-08-17 Charter 준수 감사에서 안건 ③④ 추가 — 감사 결과는 세션 로그·
> CHARTER v0.2 헤더 참조)

**A. 정량 판정 (데이터 게이트)**
- [ ] §1.4.2 옵션 C 성공 기준: #1 Brier < 0.25 / #2 트리거 적중률 ≥ 60% (`trigger_evaluations/` 12주 누적 — dt=2026-08-10 오염분 제외) / #3 portfolio outcome vs 옵션 B baseline (`portfolios/` 주간 A/B)
...
- [ ] **Sonnet 5 프롬프트 보강** — DeepEval 회귀(16/24, `signals_not_primary_evidence` 4/8 — z-score 1차 인용) 대응: 09-21 운영 결과 확인 후 bull/bear system prompt 의 signals 규칙 강화 + DeepEval 재실행(~$0.4). 확률 분포가 4.6 과 달라지면 calibration 표본 분리 검토
